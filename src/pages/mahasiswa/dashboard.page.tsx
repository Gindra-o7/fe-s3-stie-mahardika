...
const DashboardMahasiswa = () => {
  ...
  const userName = session?.user?.name || t("mahasiswa.dashboard.user.fallback");
  const welcomeText = t("mahasiswa.dashboard.welcome").replace("{name}", userName);

  const handleLogout = async () => {
    await authClient.signOut({
      fetchOptions: {
        onSuccess: () => {
          toast.success(t("mahasiswa.dashboard.logout.success"));
          navigate("/");
        },
      },
    });
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-gradient-to-br from-gray-50 via-[#207D96]/10 to-[#1B3F6E]/10 font-sans">
          ...
          <div className="flex justify-between items-center">
            <motion.div className="flex items-center gap-3" whileHover={{ scale: 1.02 }}>
              <div className="relative">
                <motion.img src={LogoMahardhika} alt={t("mahasiswa.dashboard.logo.alt")} className="w-[190px] relative object-contain" />
              </div>
            </motion.div>
      ...
      <div className="text-white flex items-center justify-center w-full h-[40px] bg-gradient-to-r from-[#207D96] to-[#1B3F6E]">
        <span>{t("mahasiswa.dashboard.copyright")}</span>
      </div>
    </div>
  );
};
